...
## The minimal hierarchy that actually works
...
**Inside Operations/**, create 4 Lists:
- "This Week" (your weekly task pool)
- "Sales Pipeline" (prospects and leads)
- "Content Calendar" (anything you publish)
- "Money" (invoices to chase, recurring expenses, tax notes)
...
## Workflow 2: Client task capture

**The job:** Capture every client request the moment it arrives so nothing falls through the cracks.

**Setup:**
1. Inside each Client Folder, the "Active Tasks" List is your inbox for that client
2. Set up ClickUp's email-to-task feature: in workspace settings → ClickApps → enable "Email in Task" → each list gets a unique forwarding address
3. Add that forwarding address as a forwarding rule in Gmail: any email from `clientdomain.com` auto-forwards a copy to `client-tasks@clickup-email.com` (gets created automatically as a task)

Alternative if email forwarding is too much: install the ClickUp Gmail extension. Reading an email from Acme, click the extension → "Create task in Acme/Active Tasks." Two clicks per request.

**Why this matters:** Every client interaction becomes a tracked task within seconds. The pattern "client mentions thing, I forget thing, client reminds me, I look bad" disappears.

## Workflow 3: The kanban for live work

**The job:** During the week, manage what you're actually executing in a simple kanban view.
...
**Why this matters:** Most solo work happens in chunks of 90 minutes to 3 hours. The Doing kanban gives you immediate visual feedback on whether you finished something this morning or just shuffled tasks around.
...
## Workflow 5: The Sales Pipeline as a List

**The job:** Stop losing prospect conversations in your inbox. Stop forgetting to follow up.

**Setup:**
1. Open "Sales Pipeline" List in Operations
2. Switch to **Board view**
3. Customize the statuses to: `Lead`, `In Conversation`, `Proposal Sent`, `Won`, `Lost`
4. Every prospect = one task with the prospect's name as title
...
- **Integrations** are richer at Unlimited

If you're billing clients hourly, the $7/month pays for itself the first time you accurately bill a half-hour you would have otherwise forgotten.

## How to actually start this week

If you've never used ClickUp:
...
The first week is about establishing the rhythm. The next 50 weeks are about not having to think about it.

## The bigger point

ClickUp's power is real and ClickUp's complexity is also real. The mistake is assuming you need to engage with the full surface area. You don't. **Use 8 Lists, 3 Views, and 5 workflows. Ship every Friday with a clean kanban. Pull a tight list every Monday.** That's a complete solo project management system, and it costs $7/month plus an hour of setup.

Most solo operators don't fail at ClickUp because the tool was wrong. They fail because they over-configured it before they ever did any actual work in it. Don't be that person. Start with the minimum, run for two weeks, and only add a feature when you've felt the absence of it twice in the same week.
